...
## SOLVED — flat E4-1 weave (both units)

The blocker was **row pitch**, not a fundamental impossibility. Earlier attempts used `py ≈ 3 mm`,
which put same-tilt neighbours 6 mm apart → fused. But the opposite-tilt **link survives out to
`dy = 6 mm`** (Lk +1, clearance 0.4–0.6), and same-tilt rings clear once **≥ 12 mm** apart. So a
row pitch of **`py = 6`** satisfies both at once, and the sheet stays **flat** (one Z, every ring
on the bed — no woven-height needed).

**Verified weave (row-brick: rows lean ±30°, odd rows staggered `px/2`, flat):**

| Unit | px | py | tilt | min clearance | interior links |
|---|---|---|---|---|---|
| round | 6.8 | 6.0 | 30° | 0.58 mm | all 4 `|Lk|=1` |
| square | 7.5 | 6.5 | 30° | 0.69 mm | all 4 `|Lk|=1` |

**Full-plate validation (`tools/build_plate.py` + `tools/sheet_scan.py`):**

| Plate | rings | footprint | fused | min clr | interlinked | watertight | kinematic ±12° |
|---|---|---|---|---|---|---|---|
| round | 957 (29×33) | 204×203 mm | 0 | 0.58 | all 4 ✓ | 957/957 ✓ | no collision ✓ |
| square | 780 (26×30) | 201×200 mm | 0 | 0.69 | all 4 ✓ | 780/780 ✓ | no collision ✓ |

Square holds clearance better through articulation (0.45 mm at −6° vs round's 0.05 mm at −12°) —
consistent with square links being looser. Params locked in `src/config.scad` (`WEAVE_*`). Plates
are generated by instancing (OpenSCAD/CGAL can't export ~1000 rings); the STLs are gitignored
(regenerate via `build_plate.py`).
